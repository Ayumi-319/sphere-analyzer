...
st.set_page_config(page_title="Sphere Analyzer v5.2", layout="wide")
st.title("🔴 スフェア自動計測ツール v5.2")
...
with st.sidebar:
    ...
    st.header("3. 二値化・切り離し設定")
    st.write(f"🎯 スポイト数: **{len(st.session_state.bg_colors)}** 点")
    sensitivity = st.slider("色の許容範囲", 1, 100, 4)
...
if uploaded_file:
    ...
    # --- 背景スポイトモードの処理 ---
    if mode == "🧪 背景スポイト吸い取り":
        st.subheader("画像内の背景（何もない所）を数点クリックしてください")
    ...
    with col_img:
        ...
        if st.session_state.manual_labels is not None:
            ax_m.contour(st.session_state.manual_labels > 0, colors='lime', linewidths=0.5)
        ax_m.axis('off')
        
        if mode == "裁断 ✂️ 手動切り離し・削除":
            st.subheader("修正モード：左クリックで分離 / 右クリックで削除")
            # 座標取得用に現在の図を画像化
            buf = io.BytesIO()
            ...
            if coords_fix and st.session_state.manual_labels is not None:
                ...
                if curr_f != st.session_state.last_coords:
                    ...
                    if 0 <= ty < data_h and 0 <= tx < data_w:
                        ...
                        if l_val > 0:
                            # 削除/分割処理 (簡易版: クリックした所を1px消して再ラベル)
                            mask_obj = st.session_state.manual_labels == l_val
                            # 周囲3pxを消去
